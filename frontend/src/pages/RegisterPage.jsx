...
export default function RegisterPage() {
  useSEO({ title: 'Crear Cuenta', url: '/registro' });
  ...
  const validar = () => {
    const nuevosErrores = {};
    if (!form.nombre.trim() || form.nombre.trim().length < 2) nuevosErrores.nombre = 'El nombre debe tener al menos 2 caracteres';
    if (!form.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) nuevosErrores.email = 'Correo inválido';
    if (form.password.length < 8) nuevosErrores.password = 'Mínimo 8 caracteres';
    if (!/[A-Z]/.test(form.password)) nuevosErrores.password = 'Debe incluir al menos una mayúscula';
    if (!/[0-9]/.test(form.password)) nuevosErrores.password = 'Debe incluir al menos un número';
  ...
  const handleSubmit = async (e) => {
    ...
    setCargando(true);
    try {
      const usuario = await registro(form);
      toast.exito(`¡Cuenta creada! Bienvenido, ${usuario.nombre} 🎉`);
      navigate('/');
    } catch (err) {
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Juan Pérez" value={form.nombre}
                  ...
                  className={`input-field pl-10 ${errores.nombre ? 'border-red-300' : ''}`}
                  autoComplete="name" />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" placeholder="tu@correo.com" value={form.email}
                  ...
                  className={`input-field pl-10 ${errores.email ? 'border-red-300' : ''}`}
                  autoComplete="email" />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={mostrarPass ? 'text' : 'password'} placeholder="Mínimo 8 caracteres"
                  ...
                  className={`input-field pl-10 pr-10 ${errores.password ? 'border-red-300' : ''}`}
